...
func NewCollector(metricsPerHost bool, registry *prometheus.Registry, buckets []float64) (Collector, error) {
	...
	s, err := collectors.NewSocketCollector(podName, podNamespace, class.IngressClass, metricsPerHost, buckets)
	if err != nil {
		return nil, err
	}

...
func TestLuaSharedDictsParsing(t *testing.T) {
	testsCases := []struct {
		...
		},
		{
			...
			expect: map[string]int{"mydict": 10000},
		},
		{
			name:   "dictionary size can not be larger than 200",
			entry:  map[string]string{"lua-shared-dicts": "mydict: 10, invalid_dict: 201, invalid_kb: 200001k"},
			expect: map[string]int{"mydict": 10000},
		},
		{
			...
			expect: map[string]int{"kb_dict_a": 512, "mb_dict_a": 30000, "kb_dict_b": 16, "mb_dict_b": 4000},
		},
	}
...
func TestDictKbToStr(t *testing.T) {
	testCases := []struct {
		...
		{
			name:   "mod 1000 reports as M",
			input:  5000,
			expect: "5M",
		},
		{
			name:   "non-mod 1000 reports as K",
			input:  5120,
			expect: "5120K",
		},
	}
